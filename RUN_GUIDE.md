...
> **Note on supervision**: `--labels_csv` / `--val_labels_csv` point at the official
> CT-RATE `*_predicted_labels.csv` files (RadBERT-generated 18-task labels shipped
> with the dataset). When provided, these CSVs are the **primary** label source —
> 100% coverage, 0 abstains. The regex extractor over `train_vqa.json` is only
> used as a fallback for records missing from the CSV.
